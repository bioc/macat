...
evalScoring <- function(data,class,chromosome,nperms=1000,permute="labels",subset=NULL,newlabels=NULL,kernel=rbf,kernelparams=NULL,cross.validate=TRUE,paramMultipliers=2^(-4:4),ncross=10,step.width=100000,memory.limit=TRUE, verbose=TRUE){
  ...
  if (permute=="labels"){
    samResult <- scoring(X,areOfClass,method="SAM",pcompute="empirical",nperms=nperms, verbose=verbose)
  } else {
