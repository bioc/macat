...
evalScoring <- function(data,class,chromosome,nperms=1000,permute="labels",subset=NULL,newlabels=NULL,kernel=rbf,kernelparams=NULL,cross.validate=TRUE,paramMultipliers=2^(-4:4),ncross=10,step.width=100000,memory.limit=TRUE){
  ...
  if (permute=="labels"){
    samResult <- scoring(X,isClass,method="SAM",pcompute="empirical",nperms=nperms)
  } else {
    samResult <- scoring(X,isClass,method="SAM",pcompute="none")
  } #else
  ...
  if (cross.validate){
    ...
    evp <- evalParams(locations=absOnChromLocs,scores=onChromScores,kernel=kernel,kernelparams=kernelparams,paramMultipliers=paramMultipliers,ncross=10,verbose=FALSE)
  ...
  if (permute=="locations"){
    ...
    #return(permSlideScores)
    #meanPermSlideScores <- rowMeans(permSlideScores)
